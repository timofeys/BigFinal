...
Сценарий:Продажа товара со склада остатка хватает
	...
	Тогда результат проведения корректен
			...
			И     элемент формы с именем "ВидДокумента" стал равен "Продажа"
			И     элемент формы с именем "СуммаИтого" стал равен "1 000"
			И     таблица формы с именем "Движения" стала равной:
			| 'N' | 'Материал'                 | 'Склад'      | 'Вид движения' | 'Количество' | 'Период'              | 'МОЛ'   |
			| '1' | 'Cвинцовая проволока 5 мм' | 'Кладовая 2' | 'Расход'       | '10,00'      | * | 'МОЛ 1' |
			| '2' | 'Асбестовый шнур 10 мм'    | 'Кладовая 2' | 'Расход'       | '1,00'       | * | 'МОЛ 1' |
